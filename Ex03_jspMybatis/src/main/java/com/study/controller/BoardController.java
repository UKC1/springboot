...
public class BoardController {
	...
	*/
	 ...
	 * 1. HttpServletRequest를 이용하여 전달받기(기존 jsp/servlet)
	 * 2. @RequestParam 어노테이션을 이용하는 방법
	 *    @RequestParam()
	 *    - 속성
	 *    value = uri에서 바인딩하게 될 값
	 *    required(true|false) : 필수적으로 값이 전달되게 되는지 안받아도 되는지(true일 떄 값이 안들어오면 에러)
	 *    defaultValue : 값이 없을 때 기본값으로 사용할 값
	 *    
	 *    - @RequestParam("전달된 name(key)") 변수명 : defaultValue를 안 넣을 때는 value는 생략가능
	 *    - 사용법
	 ...
	 * 		
	 * 		스프링컨테이너가 해당 객체를 기본생성자를 생성 후 setter 메소드를 찾아서 
	 *      전달된 값을 해당 필드에 내부적으로 담아주는 원리
	 *      - 반드시 기본생성자가 필요함
	 *      - setter메소드가 필요함
	...
	public String detailBoard(HttpServletRequest request, Model model) {
		...
		System.out.println(model.getAttribute("dBoard"));
